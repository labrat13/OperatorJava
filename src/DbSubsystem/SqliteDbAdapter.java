...
public class SqliteDbAdapter
{
    ...
    protected final static String                      TablePlaces           = "places";
    protected final static String                      TableProcs            = "routines";
    ...
    public virtual void Close()
    {
        ...
        if (((DbConnection) this.m_connection).State != ConnectionState.Closed) ((DbConnection) this.m_connection).Close();
        this.m_connection = (SQLiteConnection) null;
        this.ClearCommands();
    }

    public static String CreateConnectionString(String dbFile, boolean readOnly)
    {
    ...
    {
        if (((DbDataReader) rdr).IsDBNull(p)) return String.Empty;
    ...
    /// <remarks>
    /// Эта универсальная функция позволяет удалить строку таблицы по значению
    /// одного из ее столбцов.
    ...
    /// <param name="column">Название столбца</param>
    /// <param name="timeout">Таймаут операции в секундах</param>
    /// <returns>Возвращает максимальное значение ячеек столбца таблицы или
    /// -1.</returns>
    ...
    /// <param name="column">Название столбца</param>
    /// <param name="timeout">Таймаут операции в секундах</param>
    /// <returns>Возвращает минимальное значение ячеек столбца таблицы или
    /// -1.</returns>
    ...
    /// <param name="column">Название столбца идентификаторов записей
    /// таблицы</param>
    /// <param name="idValue">Идентификатор записи таблицы</param>
    /// <param name="timeout">Таймаут операции в секундах</param>
    /// <returns></returns>
    public boolean IsRowExists(String tablename, String column, int idValue, int timeout)
    {
    ...
    {
        using(SQLiteTransaction sqLiteTransaction=this.m_connection.BeginTransaction()){using(SQLiteCommand sqLiteCommand=new SQLiteCommand(this.m_connection)){((DbCommand)sqLiteCommand).CommandText=String.Format((IFormatProvider)CultureInfo.InvariantCulture,"DROP TABLE IF EXISTS `{0}`;",new object[1]{(object)"places"});((DbCommand)sqLiteCommand).ExecuteNonQuery();((DbCommand)sqLiteCommand).CommandText=String.Format((IFormatProvider)CultureInfo.InvariantCulture,"CREATE TABLE \"{0}\"(\"id\" Integer Primary Key Autoincrement,\"title\" Text,\"type\" Text,\"path\" Text,\"descr\" Text,\"syno\" Text);",new object[1]{(object)"places"});((DbCommand)sqLiteCommand).ExecuteNonQuery();((DbCommand)sqLiteCommand).CommandText=String.Format((IFormatProvider)CultureInfo.InvariantCulture,"DROP TABLE IF EXISTS `{0}`;",new object[1]{(object)"routines"});((DbCommand)sqLiteCommand).ExecuteNonQuery();((DbCommand)sqLiteCommand).CommandText=String.Format((IFormatProvider)CultureInfo.InvariantCulture,"CREATE TABLE \"{0}\"(\"id\" Integer Primary Key Autoincrement,\"title\" Text,\"ves\" Real,\"path\" Text,\"regex\" Text,\"descr\" Text);",new object[1]{(object)"routines"});((DbCommand)sqLiteCommand).ExecuteNonQuery();}((DbTransaction)sqLiteTransaction).Commit();}
    }
